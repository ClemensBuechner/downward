...
int LandmarkCountHeuristic::get_heuristic_value(const State &ancestor_state) {
    ...
    /*
      TODO: Check if obedient-reasonable orders may lead to goal states
       where h != 0. If so, this heuristic might wrongly report a task
       unsolvable. An explicit test for goal states is required in that
       case because we must always return 0 for a goal state.
    */
