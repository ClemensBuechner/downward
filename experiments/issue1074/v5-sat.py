...
for pref in [True, False]:
    ...
    hlm = ("landmark_sum(lm_factory=lm_reasonable_orders_hps(lm_rhw()), "
           f"transform=adapt_costs(one), pref={pref})")
...
CONFIGS = [
    ...
    IssueConfig(
        ...
        [],
        build_options=[build],
        driver_options=GLOBAL_DRIVER_OPTIONS + ["--alias", f"seq-sat-fdss-{year}"]
    )
    for build in BUILDS
...
exp.add_comparison_table_step(
    attributes=ATTRIBUTES, filter=[collect_unsupported, filter_unsupported])
