    ...
    CONFIG_NICKS.append(
        (name,
         ["--search", "--if-unit-cost",
          ...
          f"lm_rhw(only_causal_landmarks={only_causal})), pref=true), let(hff, ff(),"
          """iterated([
          ...
          f"lm_rhw(only_causal_landmarks={only_causal})),transform=adapt_costs(one),pref=true),"
          "let(hff1, ff(transform=adapt_costs(one)),"
          "let(hlm2, landmark_sum(lm_reasonable_orders_hps("
          f"lm_rhw(only_causal_landmarks={only_causal})),transform=adapt_costs(plusone),pref=true),"
