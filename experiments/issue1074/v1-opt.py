...
for only_causal in [True, False]:
    for cp in ["uniform", "optimal"]:
        name = f"lm-exhaust-{cp}"
        if only_causal:
            name += ":only-causal"
        lmc = ("landmark_cost_partitioning(lm_exhaust("
               f"only_causal_landmarks={only_causal}), cost_partitioning={cp})")
        CONFIG_NICKS.append(
            (name, ["--evaluator", f"lmc={lmc}", "--search",
                    "astar(lmc,lazy_evaluator=lmc)"])
        )

        name = f"seq-opt-bjolp-{cp}"
        if only_causal:
            name += ":only-causal"
        lmc = ("landmark_cost_partitioning(lm_merged([lm_rhw("
               f"only_causal_landmarks={only_causal}), lm_hm(m=1)]), "
               f"cost_partitioning={cp}, lpsolver=CPLEX)")
        CONFIG_NICKS.append(
            (name, ["--evaluator", f"lmc={lmc}", "--search",
                    "astar(lmc,lazy_evaluator=lmc)"])
        )
