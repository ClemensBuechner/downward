...
CONFIG_NICKS = [
    ...
    ("lmcut-hadd", ["--search", "astar(lmcut(use_hadd_instead_of_hmax=true))"], []),
]
CONFIGS = [
    IssueConfig(
        nick,
...
def make_comparison_table():
    ...
    for rev in REVISIONS:
        compared_configs.append((f"{rev}-lmcut", f"{rev}-lmcut-hadd", "Diff"))
    report = ComparativeReport(compared_configs, attributes=ATTRIBUTES)
    outfile = os.path.join(
        exp.eval_dir, f"lmcut-compare.{report.output_format}")
    report(exp.eval_dir, outfile)

exp.add_step("make-comparison-table", make_comparison_table)
