...
CONFIGS = [
    IssueConfig(f"lama-first-pref-all_interesting={all_interesting}",
                ["--evaluator",
                 "hlm=lmcount(lm_factory=lm_reasonable_orders_hps("
                 f"lm_rhw()),transform=adapt_costs(one),pref=true,all_landmarks_interesting={all_interesting})",
                 "--evaluator", "hff=ff(transform=adapt_costs(one))",
                 "--search",
                 "lazy_greedy([hff,hlm],preferred=[hff,hlm], cost_type=one,reopen_closed=false)"]) for all_interesting in [True, False]
]
...
exp.add_fetcher("data/issue1075-v1-base-lama-first-eval")
