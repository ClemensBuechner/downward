...
REVISIONS = [f"{ISSUE}-base", f"{ISSUE}-v1", f"{ISSUE}-v2"]
DRIVER_OPTIONS = ["--overall-time-limit", "5m"]
CONFIG_NICKS = [
    ("hm2-cp",
    ["--evaluator",
    "lmcp=landmark_cost_partitioning(lm_reasonable_orders_hps(lm_hm(m=2)))",
     "--search", "astar(lmcp, lazy_evaluator=lmcp)"], []),
    ("bjolp", [], ["--alias", "seq-opt-bjolp"]),
    ("bjolp-opt", ["--evaluator",
     "lmcp=landmark_cost_partitioning(lm_reasonable_orders_hps(lm_merged([lm_rhw(),lm_hm(m=1)])))",
     "--search", "astar(lmcp,lazy_evaluator=lmcp)"], []),
]
CONFIGS = [
    IssueConfig(
        config_nick,
        config,
        driver_options=DRIVER_OPTIONS + config_driver_opts)
    for config_nick, config, config_driver_opts in CONFIG_NICKS
]
...
ATTRIBUTES = IssueExperiment.DEFAULT_TABLE_ATTRIBUTES + [
    ...
    Attribute("landmarks_derived", min_wins=False),
    Attribute("orderings", min_wins=False),
    Attribute("lmgraph_generation_time", min_wins=True),
    Attribute("landmarks_dominance_pruned", absolute=True, min_wins=False),
    Attribute("orderings_deduced", absolute=True, min_wins=False),
]
